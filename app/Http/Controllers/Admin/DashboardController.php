...
class DashboardController extends Controller
{
    public function index(): Response
    {
        ...
        $paymentStatusCounts = [
            ...
            'rejected' => Transaction::where('payment_status', PaymentStatus::Rejected->value)->count(),
            'expired' => Transaction::where('payment_status', PaymentStatus::Expired->value)->count(),
            'cancelled' => Transaction::where('payment_status', PaymentStatus::Cancelled->value)->count(),
        ];

        $transactionStatusCounts = [
            'menunggu' => Transaction::where('status', TransactionStatus::Menunggu->value)->count(),
            'dibayar' => Transaction::where('status', TransactionStatus::Dibayar->value)->count(),
            'hewan_disiapkan' => Transaction::where('status', TransactionStatus::HewanDisiapkan->value)->count(),
            'tersembelih' => Transaction::where('status', TransactionStatus::Tersembelih->value)->count(),
            'didistribusikan' => Transaction::where('status', TransactionStatus::Didistribusikan->value)->count(),
        ];
